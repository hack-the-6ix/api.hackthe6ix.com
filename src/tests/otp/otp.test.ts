...
describe('OTP Controller', () => {
  ...
  describe('removeOTP', () => {
    ...
    test('should remove existing OTP', async () => {
      const result = await removeOTP(organizerUser, testOTP._id.toString());

      expect(result.success).toBe(true);
      expect(result.message).toBe('OTP removed successfully');

      const removedOTP = await OTP.findById(testOTP._id);
      expect(removedOTP).toBeNull();
    });

    test('should not remove non-existent OTP', async () => {
      const fakeId = '507f1f77bcf86cd799439011';
      const error = await getError<NotFoundError>(() =>
        removeOTP(organizerUser, fakeId),
      );
